...
def reid_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            ...
            detections[frame_meta.source_id].append({
                "bbox": np.array([
        ...
        with nvtx.annotate("tracker_update", color="red"):
            all_tracks1= tracker1.update(detections[0])
            all_tracks2= tracker2.update(detections[1])
            
            print(f"[DS] : {len(detections[0]), len(detections[1])}")
        
            # mct.update_global(trackers=[tracker1, tracker2])
